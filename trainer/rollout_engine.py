...
class RolloutEngine(ABC):
    ...
    def rollout(self, prompt_ids: Tensor, attention_mask: Tensor, num_generations: int, max_new_tokens: int,
                temperature: float = 0.8, top_p: Optional[float] = None,
                top_k: Optional[int] = None, calculate_logps: bool = True) -> RolloutResult:
        pass
        ...
        with torch.no_grad(), ctx:
            ...
            if calculate_logps:
                ...
                per_token_logps = torch.zeros(
                    output_ids.size(0), completion_ids.size(1), device=output_ids.device, dtype=torch.float32
                )
